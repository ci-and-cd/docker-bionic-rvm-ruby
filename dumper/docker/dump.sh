...
for layer in $(pwd)/data/image/*/layer.tar; do
    ...
    if [ -n "$(tar -tf ${layer} | grep '\.rvm')" ]; then echo found .rvm in ${layer}; LAYERS+=(${layer}); fi
done
...
tar_directories=($(tar tf data/layer.tar | grep -E '.*/$' | grep -v '\.rvm' | sort -r -n))
...
find /data/root/home/ubuntu ! -path '/data/root/home/ubuntu' | grep -v '.rvm' | grep -v '.gem' | xargs rm -rf
